...
function HeaderTabBar(
  ...
  return (
    ...
    >
      ...
      <MaterialTopTabBar
        {...(barProps as any)}
        style={[((barProps as any).style), styles.blurredBar]}
...
export default function TopTabsNavigator() {
  ...
  const pickVideo = async () => {
    ...
    if (!result.canceled) {
      const uri = result.assets[0].uri;
      const info: FileSystem.FileInfo = await FileSystem.getInfoAsync(uri, { size: true });

      if (info.size != null && info.size > 20 * 1024 * 1024) {
