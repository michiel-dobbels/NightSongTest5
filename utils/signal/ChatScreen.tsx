...
const ChatScreen = ({ route }: { route: ChatScreenRouteProp }) => {
  ...
  const signalStore = useSignal(); // or however you're initializing it

  if (!signalStore) {
    console.warn('No signal store available');
    return;
  }
    ...
    const decrypted = await Promise.all(
      data.map(async (msg) => {
        try {
          ...
          return {
            ...msg,
            decryptedText: new TextDecoder('utf-8').decode(plaintextBytes),
          };
        } catch (e) {
